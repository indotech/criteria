...
### Библиотечная функция или метод объекта изначально не созданные как обработчик события или колбэк, не должны ими становиться
**Правильно**
```js
...
### Имена приватных и защищенных методов начинаются с нижнего подчеркивания

### Вызовы приватных методов в наследниках минимизированы
Это значит, что наследники должны стараться пользоваться только публичным API класса.

### Метод который изменяет внутреннее состояние объекта ничего не возвращает или возвращает `this` для чейнинга. Исключение асинхронные функции, которые всегда возвращают Промис
Это сложный и спрорный критерий. Его соблюдение должно сделать код более предсказуемым,
и освободить голову от мыслей, что разработчики будут заучивать как ведет себя API вашего класса.

Будьте аккуратны с этим критерием, его точно стоит обсудить несколько раз и запечатлить эти обсуждения тут.
Оставьте его пока последним.
